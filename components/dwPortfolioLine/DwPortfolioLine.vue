 ...
 * @LastEditTime: 2022-01-12 13:48:08
 * @FilePath: /dw-vue-components/components/dwPortfolioLine/DwPortfolioLine.vue
 * @Description: 西筹“基金组合”结果页优化折线图
-->
<template>
    <div class="dw-portfolio-line">
        <VChart class="dw-portfolio-chart" :option="option" :style="chartStyle" />
    </div>
</template>

<script setup lang="ts">
import { defineProps, reactive, provide, computed, watch } from 'vue'
...
const createIndex = computed(() => {
    // const index = props.xData.indexOf(formatterCreateDate.value)
    const index = props.xData.indexOf(props.createDate)
    return index
})
// y轴数据监听
const lineYData = reactive(props.yData)
// echarts option
    ...
    series: [
        {
                ...
                data: [
                    {
                        ...
                        yAxis:
                            props.createPoint && props.currCheckedIndex == 1
                                ? lineYData.lineOneData[createIndex.value]
                                : '',
                    },
                ],
                label: {
            ...
            data: lineYData.lineOneData.map((value, index) => {
                if (
        ...
        },
        {
            ...
            data: lineYData.lineTwoData.map((value, index) => {
                return {
                    value,
                    itemStyle: {
                        color: '#589DFC',
